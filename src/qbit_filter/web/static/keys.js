(function () {
  ...
  const saveCacheNow = () => {
    ...
    try {
      localStorage.setItem(CACHE_KEY, groups.innerHTML);
      setCacheCookie(true);
    } catch (e) {
  ...
  /* =========================================================================
     ...
     the in-memory `selection` Set; after every relevant swap we re-paint
     checkboxes & data-marked attributes from the set so selections survive
     partial re-renders.
     ========================================================================= */
  ...
  function repaintSelectionFooter() {
    ...
    if (sizeEl) {
      ...
      for (const h of selection) {
        const row = document.querySelector(`.torrent-row[data-hash="${h}"]`);
        if (!row) continue;
        /* Prefer the explicit data-bytes attribute on the size span; fall
           back to scraping the GB text for rows from cached HTML predating
           the attribute. */
        ...
        for (const span of metaSpans) {
          ...
          if (m) { bytes += parseFloat(m[1]) * 1073741824; break; }
        }
      }
  ...
  document.body.addEventListener('change', (e) => {
    ...
    if (!h) return;
    if (t.checked) selection.add(h); else selection.delete(h);
  ...
  /* After a fresh swap into #groups (filter POST, rule preview, SSE RESYNC),
     ...
     set and reflect that in checkbox state. The user can deselect mistakes
     before confirming bulk-delete. */
  document.body.addEventListener('htmx:afterSwap', (e) => {
    const target = e.detail && e.detail.target;
    if (!target) return;
    const isRelevant = target.id === 'groups'
      || target.id === 'rule-bar-slot'
      || (target.querySelector && target.querySelector('.group-card'));
    if (!isRelevant) return;
    document.querySelectorAll('.torrent-row[data-marked="true"]').forEach((row) => {
      const h = row.getAttribute('data-hash');
      if (h) selection.add(h);
  ...
  (function viewportObserver() {
    ...
    const attach = () => {
      document.querySelectorAll('.group-card').forEach((el) => obs.observe(el));
    };
